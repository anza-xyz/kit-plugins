...
```json
{
  "peerDependencies": {
    "react": "^18.0.0 || ^19.0.0",
    "@solana/kit": "^6.x",
    "@solana/kit-plugin-rpc": "^1.x",
    "@solana/kit-plugin-wallet": "^1.x"
  }
}
```

Zero dependencies beyond React and Kit. No cache library required.
...
Each provider reads the client from context, extends it with `.use()`, and provides the new client. The nesting order is the plugin chain order — each provider maps to a Kit plugin.
...
**`WalletProvider`** — wraps one of the wallet plugins. Accepts an optional `role` prop that defaults to `"signer"` — the common case where the wallet both pays for and signs transactions. Other roles are available for advanced setups: `"payer"` (wallet pays fees only), `"identity"` (wallet signs but a relayer pays), or `"none"` (wallet UI only, manual signer access via `useConnectedWallet()`). These map to the `walletSigner`, `walletPayer`, `walletIdentity`, and `walletWithoutSigner` plugins respectively. Also accepts `autoConnect`, `storage`, `storageKey`, and `filter` props, forwarded to the underlying wallet plugin.
...
// Testing with LiteSVM — swap RpcProvider for LiteSvmProvider
 ...
 */
function useBalance(address: Address): LiveQueryResult<Lamports>;
 ...
 */
function useAccount(address: Address): LiveQueryResult<EncodedAccount | null>;
...
Where `useLiveStore` is an internal helper that manages store creation, abort, and cleanup:
